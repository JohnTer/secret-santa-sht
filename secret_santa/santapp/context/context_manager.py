...
from .contexts import MenuContext, WrittingAddressContext, WrittingWishlistContext, TutorialContext
...
class ContextManager(object):
    def __init__(self):
        ...
        self.contexts: dict[str, Union[MenuContext, WrittingAddressContext,
                                       WrittingWishlistContext]] = {
                                           UserStateEnum.TUTORIAL:
                                           tutorial_ctx,
    ...
    async def get_message_object(self, json_data: dict) -> UserMessage:
        ...
        payload: dict[str, str] = orjson.loads(json_message['payload']) if 'payload' in json_message else None
    ...
    async def _run_context_manager(self, message: UserMessage) -> None:
        ...
        logging.info(f'The user [vk_id={message.user.vk_id}] has the state={state}')
